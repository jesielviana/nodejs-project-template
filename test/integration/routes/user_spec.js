/* global describe before beforeEach afterEach it context setupApp supertest expect */
...
describe('Routes: users', () => {
  ...
  beforeEach(() => {
    ...
    user._id = defaultId
    return User.deleteMany({}).then(() => user.save())
  })
  ...
  describe('GET /api/users', () => {
    it('should return a list of users', done => {
      request.get(`${Config.API_BASE}/users`).end((err, res) => {
    ...
    context('when an id is specified', () => {
      it('should return 200 with one User', done => {
        request.get(`${Config.API_BASE}/users/${defaultId}`).end((err, res) => {
